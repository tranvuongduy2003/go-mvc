...
func NewBaseJob(jobType string, payload jobs.JobPayload) *BaseJob {
	return &BaseJob{
		id:         uuid.New(),
		jobType:    jobType,
		payload:    payload,
...
func NewBaseJobWithOptions(jobType string, payload jobs.JobPayload, opts jobs.JobOptions) *BaseJob {
	job := &BaseJob{
		id:          uuid.New(),
		jobType:     jobType,
		payload:     payload,
...
func (j *BaseJob) GetPayload() jobs.JobPayload {
	return j.payload
}

// GetPriority returns the job's priority
func (j *BaseJob) GetPriority() jobs.JobPriority {
	return j.priority
}

// GetStatus returns the current status of the job
func (j *BaseJob) GetStatus() jobs.JobStatus {
	return j.status
}

// SetStatus updates the job's status
func (j *BaseJob) SetStatus(status jobs.JobStatus) {
	j.status = status
}
...
func FromMap(data map[string]interface{}) (*BaseJob, error) {
	...
	job := &BaseJob{
		...
		payload:    data["payload"].(jobs.JobPayload),
		priority:   jobs.JobPriority(data["priority"].(int)),
		status:     jobs.JobStatus(data["status"].(string)),
